...
(async () => {
    ...
    let proxy = "https://scrapper-proxy.herokuapp.com";
    ...
    response1 = await fetch(proxy, { ...options, method: "POST", body: JSON.stringify({ url:merchant_url  }) }, timeout);
    res1 = await response1.json();
    response2 = await fetch(proxy, { ...options, method: "POST", body: JSON.stringify({ url:product_url  }) }, timeout);
